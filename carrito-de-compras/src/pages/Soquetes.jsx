...
const { data, setSelectedProduct } = useContext(ProductContext);
  ...
  const soquetesData = data.filter((product) => product.category === 'soquetes');
  return (
    <>
    <div className="product-cont">
        {soquetesData.map((soquete) => (
          ...
          >
            ...
            <NavLink
                  ...
                  to={`/productHome/${soquete.id}`} // Actualiza la ruta con el ID del producto
                  onClick={() => redirectToProductHome(soquete.id)}
                >
                    <h2>{soquete.nameProduct}</h2>
                
                </NavLink>
                <div className="product-footer">
                <div className="product-photo">
              <img src={soquete.urlImage} alt={soquete.nameProduct} />
              </div>
              <div className="info-product">
               
                <p><strong>{soquete.price}</strong></p>
                <button
                  onClick={() => addProduct(soquete)}
                  style={{
                    background: addedProduct(soquete.id) ? '#5ccb5f' : soquete.stock > 0 ? '' : 'red',
                    color: addedProduct(soquete.id) || soquete.stock > 0 ? 'white' : 'white'
                  }}
                  disabled={addedProduct(soquete.id) || soquete.stock <= 0}
                >
                  {addedProduct(soquete.id) ? "En el carrito" : soquete.stock > 0 ? "Añadir al carrito" : "Sin stock"}
                </button>
                </div>
              </div>
