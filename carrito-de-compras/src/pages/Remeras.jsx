        ...
        {remerasData.map((remera) => (
          ...
          >
            ...
            <NavLink
                  ...
                  to={`/productHome/${remera.id}`} // Actualiza la ruta con el ID del producto
                  onClick={() => redirectToProductHome(remera.id)}
                >
                    <h2>{remera.nameProduct}</h2>
                
                </NavLink>
                <div className="product-footer">
                <div className="product-photo">
              <img src={remera.urlImage} alt={remera.nameProduct} />
              </div>
              <div className="info-product">
               
                <p><strong>{remera.price}</strong></p>
                <button
                  onClick={() => addProduct(remera)}
                  style={{
                    background: addedProduct(remera.id) ? '#5ccb5f' : remera.stock > 0 ? '' : 'red',
                    color: addedProduct(remera.id) || remera.stock > 0 ? 'white' : 'white'
                  }}
                  disabled={addedProduct(remera.id) || remera.stock <= 0}
                >
                  {addedProduct(remera.id) ? "En el carrito" : remera.stock > 0 ? "Añadir al carrito" : "Sin stock"}
                </button>
                </div>
              </div>
