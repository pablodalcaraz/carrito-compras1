...
const { data, setSelectedProduct } = useContext(ProductContext);
  ...
  const mediasData = data.filter((product) => product.category === 'medias');
  return (
    <>
        ...
        {mediasData.map((media) => (
          ...
          >
            ...
            <NavLink
                  ...
                  to={`/productHome/${media.id}`} // Actualiza la ruta con el ID del producto
                  onClick={() => redirectToProductHome(media.id)}
                >
                    <h2>{media.nameProduct}</h2>
                
                </NavLink>
                <div className="product-footer">
                <div className="product-photo">
              <img src={media.urlImage} alt={media.nameProduct} />
              </div>
              <div className="info-product">
               
                <p><strong>{media.price}</strong></p>
                <button
                  onClick={() => addProduct(media)}
                  style={{
                    background: addedProduct(media.id) ? '#5ccb5f' : media.stock > 0 ? '' : 'red',
                    color: addedProduct(media.id) || media.stock > 0 ? 'white' : 'white'
                  }}
                  disabled={addedProduct(media.id) || media.stock <= 0}
                >
                  {addedProduct(media.id) ? "En el carrito" : media.stock > 0 ? "Añadir al carrito" : "Sin stock"}
                </button>
                </div>
              </div>
