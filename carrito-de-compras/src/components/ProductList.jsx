        ...
        {data.map((product) => (
          ...
          >
            ...
            <NavLink
                  ...
                  to={`/productHome/${product.id}`} // Actualiza la ruta con el ID del producto
                  onClick={() => redirectToProductHome(product.id)}
                >
              ...
              <img style={{borderRadius:'5px'}} src={product.urlImage} alt={product.nameProduct} />
              </div>
              ...
              <p><strong>${Number(product.price).toFixed(2)}</strong></p>
                ...
                <button
                  ...
                  style={{
                    background: addedProduct(product.id) ? '#5ccb5f' : product.stock > 0 ? '' : 'red',
                    color: addedProduct(product.id) || product.stock > 0 ? 'white' : 'white'
                  }}
                  disabled={addedProduct(product.id) || product.stock <= 0}
                >
                  {addedProduct(product.id) ? "En el carrito" : product.stock > 0 ? "Añadir al carrito" : "Sin stock"}
                </button>
                </div>
              </div>
